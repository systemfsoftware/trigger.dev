...
The management API is available through the same `@systemfsoftware/trigger.dev_sdk` package used in defining and triggering tasks. If you have already installed the package in your project, you can skip this step.
...
```bash npm
npm i @systemfsoftware/trigger.dev_sdk@beta
```

```bash pnpm
pnpm add @systemfsoftware/trigger.dev_sdk@beta
```

```bash yarn
yarn add @systemfsoftware/trigger.dev_sdk@beta
```
...
All `v3` functionality is provided through the `@systemfsoftware/trigger.dev_sdk/v3` module. You can import the entire module or individual resources as needed.

```ts
import { configure, runs } from "@systemfsoftware/trigger.dev_sdk/v3";
...
```ts
import { configure, envvars } from "@systemfsoftware/trigger.dev_sdk/v3";
...
```ts
import { runs, APIError } from "@systemfsoftware/trigger.dev_sdk/v3";
